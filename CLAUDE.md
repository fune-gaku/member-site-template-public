...
- **[deployment.md](.claude/deployment.md)** - デプロイ手順（Cloudflare Workers、必須項目のみ）
- **[deployment-optional.md](.claude/deployment-optional.md)** - 任意機能（opt-in）セットアップ：Cloudflare Turnstile / Google OAuth など
...
| `PUBLIC_TURNSTILE_SITE_KEY`       | Cloudflare Turnstile site key（公開値、任意）   | Cloudflare Dashboard > Turnstile > Add Site<br/>**デフォルト OFF**: 未設定なら widget 非表示。`supabase/config.toml` `[auth.captcha].enabled = true` + 本番 Dashboard ON の 3 層が揃ったとき opt-in で有効化                                                                                                                                                                                                                                                                                        |
| `TURNSTILE_SECRET_KEY`            | Cloudflare Turnstile secret key（秘密値、任意） | 同上の secret key 欄<br/>**デフォルト OFF** で設定不要。ON にする場合のみ — ローカル: **プロジェクトルート `.env`**（`supabase/config.toml` の `[auth.captcha]` が `env(...)` で解決）／本番: **Supabase Dashboard > Authentication > Settings > Bot and Abuse Protection** に登録（Workers secret は不要）                                                                                                                                                                                         |
...
| `PUBLIC_GOOGLE_AUTH_ENABLED`      | Google OAuth ログインを有効化（公開値、任意）   | `true` で有効化（signin/signup に Google ボタン表示 + `auth.signInWithGoogle` Action 受付）。**デフォルト OFF**（未設定 / `false` で UI 非表示 + Action は `NOT_FOUND`）。有効化前に Google Cloud Console + Supabase Dashboard > Auth > Providers > Google の設定が必要（手順は [.claude/deployment-optional.md「Google OAuth セットアップ（任意）」](.claude/deployment-optional.md#google-oauth-セットアップ任意)）。**Issue #49 で進行中**: コード（Action / UI）は PR 2 / PR 3 マージ後に有効化 |
