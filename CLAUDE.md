...
| `TURNSTILE_SECRET_KEY`            | Cloudflare Turnstile secret key（秘密値、任意） | 同上の secret key 欄<br/>ローカル: **プロジェクトルート `.env`**（`supabase/config.toml` の `[auth.captcha]` が `env(...)` で解決）<br/>本番: **Supabase Dashboard > Authentication > Settings > Bot and Abuse Protection** に登録（Workers secret は不要、Supabase Auth が直接検証） |
