...
| 階層                           | 意図                                                | 代表例                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------------------ | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **error**（バグ直結）          | AI 生成バグの早期検出に直結。違反 0 件で error 維持 | `@typescript-eslint/no-floating-promises` / `no-misused-promises` / `no-base-to-string` / `require-await` / `no-unsafe-*`（argument / assignment / call / member-access / return）/ `no-deprecated` / `no-unnecessary-condition` / `no-unnecessary-type-assertion` / `restrict-template-expressions` / `prefer-regexp-exec` / `prefer-optional-chain` / `no-confusing-void-expression` / `no-explicit-any` / `no-non-null-assertion` / `consistent-type-imports` / `import/order` / `eqeqeq` / `no-shadow` / `no-throw-literal` |
| **warn**（段階導入）           | 既存違反 100+ 件のクラス。Sub Issue で段階格上げ    | `@typescript-eslint/method-signature-style` / `complexity` / `max-depth` / `prefer-template` 等のスタイル系（Issue #38 リリース時の暫定段階。`--max-warnings 0` 到達後は新規違反が即 fail なので段階導入リスクは小）                                                                                                                                                                                                                                                                                                            |
...
現在の baseline は [package.json](../package.json) の `lint` スクリプトで管理（Issue #38 リリース時 151 → #56 で 147 → #57 で 123 → #59 一部 (`eslint --fix` 自動修正分) で 84 → typed lint クラスタ解消で 77 → .astro consistent-return off で 69 → prefer-destructuring 解消で 63 → tests の no-explicit-any off で 52 → id-length exceptions 拡充で **0**）。**最終目標 `--max-warnings 0` 達成**。
