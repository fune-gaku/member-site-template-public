...
| 階層                           | 意図                                                | 代表例                                                                                                                                                                                                                                                         |
| ------------------------------ | --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **error**（バグ直結）          | AI 生成バグの早期検出に直結。違反 0 件で error 維持 | `@typescript-eslint/no-floating-promises` / `no-misused-promises` / `no-base-to-string` / `require-await` / `no-explicit-any` / `no-non-null-assertion` / `consistent-type-imports` / `import/order` / `eqeqeq` / `no-shadow` / `no-throw-literal`             |
| **warn**（段階導入）           | 既存違反 100+ 件のクラス。Sub Issue で段階格上げ    | `@typescript-eslint/no-unsafe-*`（argument / assignment / call / member-access / return）/ `no-deprecated` / `no-unnecessary-condition` / `restrict-template-expressions` / `method-signature-style` / `complexity` / `max-depth` / `id-length` 等のスタイル系 |
