...
| **warn**（段階導入）           | 既存違反 100+ 件のクラス。Sub Issue で段階格上げ    | `@typescript-eslint/method-signature-style` / `complexity` / `max-depth` / `id-length` / `consistent-return` / `prefer-destructuring` 等のスタイル系                                                                                                                                                                                                                                                                                                                                                                            |
...
現在の baseline は [package.json](../package.json) の `lint` スクリプトで管理（Issue #38 リリース時 151 → #56 で 147 → #57 で 123 → #59 一部 (`eslint --fix` 自動修正分) で 84 → typed lint クラスタ解消で 77 → .astro consistent-return off で 69）。
