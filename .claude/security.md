...
- [x] CAPTCHA (Cloudflare Turnstile) インフラ：`auth.signUp` / `auth.signIn` / `auth.resetPassword` の 3 経路すべてで Supabase Dashboard と `supabase/config.toml` の両側で有効化されているとき opt-in で適用される。**本テンプレートのデフォルトは OFF**（[supabase/config.toml](../supabase/config.toml) の `[auth.captcha].enabled = false` + 本番 Dashboard も OFF 想定）。bot 対策が必要なプロジェクトで 3 層を揃えて再有効化する。bot による credential stuffing / 自動アカウント作成 / spam reset を抑止（`src/components/TurnstileWidget.vue` を 3 フォーム共通で使用 — Issue #21）。loader script 取得失敗 (ad blocker / CSP / network) は `script.onerror` + 10s timeout で graceful 化し、`@loader-error` emit を通じて各フォームでユーザ向け instruction を表示（Issue #30）。検証は **Supabase Auth (GoTrue) の公式機能** に委譲し（Issue #52）、Action は `captchaToken` を `supabase-js` の `options.captchaToken` に流すだけの薄い経路。`captcha_failed` (`error.code`) は enumeration 統一応答に巻き込まずに `BAD_REQUEST` で個別エラーとして返す（bot 検知失敗は enumeration vector ではないため）。設定手順は [.claude/deployment.md「Cloudflare Turnstile（任意 / bot 対策）」](./deployment.md#cloudflare-turnstile任意--bot-対策)
...
- [ ] **進行中（Issue #49）**: Google OAuth ログイン（PKCE フロー）インフラ。**PR 1（本 PR）は基盤・ドキュメント先行**（`PUBLIC_GOOGLE_AUTH_ENABLED` env + Cloud Console / Supabase Dashboard セットアップ手順 + 脅威モデル更新）。コード本体は PR 2（`auth.signInWithGoogle` Action + `/auth/callback` の `exchangeCodeForSession` 経路）/ PR 3（signin/signup UI の SSR 条件分岐 + パスワード変更 UI を email provider 持ちのみに限定）で追加予定。設計方針: 本テンプレートのデフォルトは OFF（`PUBLIC_GOOGLE_AUTH_ENABLED` 未設定 / `false` で UI 非表示 + Action は `NOT_FOUND` 相当）。identity linking は Supabase デフォルトの **automatic linking** に委ねるが、**前提条件として Email confirmation = ON が必須**（[Supabase Dashboard セキュリティ設定チェックリスト](#supabase-dashboard-セキュリティ設定チェックリスト) の「Email confirmation: ON」を維持）。これにより両 identity が確認済 email である状態でしか自動リンクが起きず、未確認 email を踏み台にした pre-account takeover を防ぐ（[Identity Linking](https://supabase.com/docs/guides/auth/auth-identity-linking)）。要求スコープは Supabase デフォルトの `openid email profile` のみで Drive / Calendar 等は要求しない（最小権限）。セットアップ手順は [.claude/deployment.md「Google OAuth セットアップ（任意）」](./deployment.md#google-oauth-セットアップ任意)
...
| Credential stuffing             | 中           | `auth.signIn` に Cloudflare Turnstile を opt-in 適用可能（Dashboard + `[auth.captcha]` 両方 ON で有効化、本テンプレデフォルトは OFF）— Issue #21 / #52（検証は Supabase Auth に委譲）                                                                                                                                                                                                                                                   |
| 自動アカウント作成 / Spam reset | 中           | `auth.signUp` / `auth.resetPassword` にも Turnstile を opt-in 適用可能（同上）— Issue #21 / #52                                                                                                                                                                                                                                                                                                                                         |
| OAuth pre-account takeover      | 中           | 攻撃者が未確認の被害者 email で先にローカル signup → 被害者が後から Google OAuth で同 email を初回ログインしたとき identity が攻撃者アカウントに linkage される脅威。本テンプレは **Email confirmation = ON 前提**（Supabase Dashboard） + Supabase の automatic linking が「両 identity が確認済 email」状態でのみ動く仕様に依拠して防ぐ（[Identity Linking](https://supabase.com/docs/guides/auth/auth-identity-linking)）— Issue #49 |
