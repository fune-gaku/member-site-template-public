...
**ステータス**: Open
**カテゴリ**: Bug / Security
**伴走 Issue**: [Issue #002-B](./002_b_password_reset_update_password_missing.md)（パスワードリセット: 新パスワード設定画面と updateUser Action）

---

## 本 Issue の位置付け

メール認証フロー全体の破綻を扱う 2 件の兄弟 Issue のうちの 1 つ。並走で進める前提。

| Issue      | 扱う範囲                                                                                          |
| ---------- | ------------------------------------------------------------------------------------------------- |
| **#002**（本 Issue） | メール内リンクが scanner プリフェッチで OTP 消費される問題 → `/auth/confirm` ランディングページ新設 |
| **#002-B** | verifyOtp 後の **新パスワード設定画面と updateUser Action 自体が存在しない** 問題                  |

#002 を完了しても **#002-B を直さなければパスワードリセットは依然として機能しない**（dashboard に飛ばされるだけ）。逆に #002-B を先に実装しても scanner 問題で OTP が消費されて画面まで到達できない。**両方セットで完了が必須**。
...
- 同じく OTP 方式を使う **パスワードリセットメール** (`/auth/reset-password`) も同じ scanner 消費問題を抱える（[Issue #002-B](./002_b_password_reset_update_password_missing.md) で本 Issue の `/auth/confirm` を共用する前提）
