...
会員サイトテンプレート。Astro + Vue + Supabase + Cloudflare Workers で、認証 / プロフィール / 投稿 / 管理画面 / RLS / セキュリティヘッダを初期実装済み。コード変更・機能追加は同梱の Claude Code 規約（[CLAUDE.md](CLAUDE.md) / `.claude/*.md`）に従う前提で運用する。

> ⚠️ **Supabase の Service Role key などの秘密値は、ユーザー自身がエディタで `.env` / `.dev.vars` に直接書き込みます**。チャット欄に貼ると会話履歴に残るリスクがあるため、Claude Code には渡さない運用です（プレースホルダや `.env.example` の編集は Claude Code に任せて OK）。
...
GUI 操作（Dashboard）とターミナルへのコマンドコピペを交互に行う。

### Step 1: GitHub で自分のリポジトリを作る（GUI）

このリポジトリのページ右上、緑色の **"Use this template"** ボタンをクリックし、**"Create a new repository"** を選ぶ。

- **Repository name**: 任意（例: `my-club`）
- **Public / Private**: どちらでも可（迷ったら Private 推奨）

**"Create repository"** をクリックして数秒待つと、自分のリポジトリができあがる。次の Step に進む前に、リポジトリページ右上の緑色の **"Code"** ボタン → **"HTTPS"** タブで表示される URL（`https://github.com/<your-name>/<repo>.git`）をコピーしておく。Step 3-1 の `git clone` コマンドで使う。
...
3. 以下を入力して **"Create new project"** をクリック:
   ...
   - **Database Password**: ランダムな強いパスワードを生成し、**パスワードマネージャに保存**（後の本番運用で使う）
   - **Region**: 利用者が多い地域に近い場所（日本なら **"Northeast Asia (Tokyo)"**）
4. プロジェクトが **"Setting up project"** 状態になる → 1〜2 分待つ

完成したら、左サイドバーの **歯車アイコン (Project Settings) → API** を開く。以下 3 つの値を **パスワードマネージャに保存** する。**用途は本番デプロイ（Step 6〜7）のみで、ローカル開発では使わない** — ローカル開発は Step 3-2 で起動する Docker 上のローカル Supabase が独自の URL / キーを発行するので、それを `.env` / `.dev.vars` に書く:
...
> Service Role key の右にある 👁 アイコンを押すと値が表示される。コピー後、画面は早めに閉じる。

> ⚠️ **本番値をローカル `.env` に入れない** — `src/lib/supabase.ts` は `import.meta.env.PUBLIC_SUPABASE_URL` をそのまま使うため、本番 Project URL を local `.env` に書くと `npm run dev` でも本番 Supabase に接続してしまい、テストデータが本番に書き込まれる。Service Role key も同様に local `.dev.vars` に入れると admin 系 Action が本番 DB に対して service role 権限で動作する危険がある。ローカル開発では Step 3-2 で得る **ローカル Supabase の URL / キー** だけを使う。

### Step 3: ローカル開発環境を立ち上げる（ターミナル + エディタ）

PC のお好きな場所（例: `~/Developer/`）でターミナルを開き、以下を順に実行する。Step 2 で控えた本番値はここでは **使わない**。

#### 3-1. リポジトリを clone して依存関係をインストール（ターミナル）

`<Step 1 でコピーした URL>` の部分だけ自分の値に置き換える。
...
最後の 2 行で、空の `.env` と `.dev.vars` がリポジトリルートに作られる（次のステップで値を埋める）。

#### 3-2. ローカル Supabase を起動して URL / キーを取得（ターミナル）

Docker Desktop を起動した状態で、リポジトリのルートで以下を実行する。

```bash
npm run db:start
```

初回は Docker image 取得で 1〜3 分かかる。完了すると、ターミナルに以下のような **ローカル Supabase の URL とキー** が表示される:
...
> 出力を消してしまった場合は `npx supabase status` で再表示できる。

ここで表示される **API URL / Publishable key（または anon key）/ service_role key** が、次のステップで `.env` / `.dev.vars` に書く値。**Step 2 の本番値は使わない。**

#### 3-3. `.env` と `.dev.vars` に Step 3-2 の **ローカル値** を書き込む（エディタ）

clone したリポジトリをお好みのエディタ（VS Code 等）で開き、以下 2 ファイルを編集する。**Service Role key などの秘密値は Claude Code のチャット欄には貼らず、自分でファイルに直接書き込む**（会話履歴に残さないため）。
...
`.env` / `.dev.vars` はどちらも `.gitignore` 対象なので、誤ってコミットされることはない。

> ⚠️ **Step 2 の本番値（`https://<ref>.supabase.co` 形式の URL や hosted Service Role key）をここに書かない** — local の `.env` に本番値を入れると、`npm run dev` 起動時にアプリが本番 Supabase に接続し、テストデータが本番 DB に書き込まれる。Step 2 の値は Step 6〜7（Cloudflare Dashboard 登録時）でのみ使う。
...
`npm run dev` がローカル URL（通常 <http://localhost:4321>）を表示したら成功。

### Step 4: ブラウザで動作確認（GUI）

Step 3-4 の `npm run dev` が表示するローカル URL（通常 <http://localhost:4321>）をブラウザで開いて、以下を確認する:
...
ローカル DB は Docker 内で完結しているので、ここで作ったテストアカウントは本番の Supabase には影響しない。サインアップ確認メールはローカルのメールキャッチャー（`http://localhost:54324`、Inbucket）で確認できる。

---

## 本番デプロイ

ローカルで作ったコードを **本番の Supabase + Cloudflare Workers** に展開する。Cloudflare の **Workers Builds** という GitHub 連携機能を使うと、`git push` ごとに Cloudflare 側が自動でビルド・デプロイする（手元で `wrangler deploy` を打つ必要なし）。

### Step 5: Supabase 本番セキュリティ設定（GUI）

本番では Step 2 で作成した hosted Supabase プロジェクトを使う（ローカル開発で使った Docker 上のローカル Supabase は本番には関係しない）。Step 2 のときに保存した Project URL / Publishable key / Service Role key をここから先で使う。

Supabase Dashboard で以下 2 箇所を設定する。
...
サインアップ確認メール / パスワードリセットメール / メール変更メールの本文を、自社の名前と署名に置き換える。**`{{ .ConfirmationURL }}` は使わず、`{{ .TokenHash }}` ベースの `/auth/confirm` 経由のリンク**にする必要がある（プリフェッチ・スキャナ耐性のため）。

完全な設定値・テンプレ HTML サンプルは [.claude/deployment.md「セキュリティ設定」](.claude/deployment.md#セキュリティ設定) に揃っている。Claude Code に「`.claude/deployment.md` のセキュリティ設定セクションに従って Supabase Dashboard 用のメールテンプレ HTML を出して」と頼めば、コピペ可能な形で出力される。

その他の Dashboard 設定（OTP expiry / Sessions / MFA など）も同ドキュメントの **本番環境チェックリスト** に集約されている。

Supabase Advisor で `Leaked Password Protection Disabled` 警告が出ている場合は、Free プランの仕様で Dashboard 機能が使えないだけ（バグではない）。本テンプレートはアプリ層フォールバック（`ENABLE_HIBP_CHECK=true` で HIBP API 照会）を備えているので、警告は **想定内** として扱って問題ない。プラン別の対応方針・Pro 移行時のスイッチング手順は [.claude/security-ops.md「Supabase Advisor で残る想定済み警告と対応」](.claude/security-ops.md#supabase-advisor-で残る想定済み警告と対応) を参照。

### Step 6: Cloudflare Workers にデプロイ（GUI）

Cloudflare 公式の **Workers Builds**（GitHub 連携で自動デプロイ）を使う。Cloudflare には **ビルド時の env**（Vite が `astro build` 中に読む）と **ランタイムの env**（Worker が本番リクエストで `env.X` として読む）の 2 系統があり、登録場所が分かれている。Step 6 でビルド時を、Step 7 でランタイムを設定する。
...
同じ画面の **"Build variables and secrets"** セクションを展開し、`PUBLIC_*` の **3 つだけ** を登録する。これらは **ビルド時** に Vite がバンドルへ inline する値。
...
> ⚠️ **`SUPABASE_SERVICE_ROLE_KEY` をここに書かない**。Build variables は **ビルド中だけ** 有効で、Workers ランタイムには引き継がれない（[Cloudflare 公式](https://developers.cloudflare.com/workers/ci-cd/builds/configuration/)）。誤って書いてもビルドは通るが、本番リクエストで `env.SUPABASE_SERVICE_ROLE_KEY` が `undefined` になり、admin 機能が全滅する。Service Role key は Step 7 で別の場所に登録する。

#### 6-3. 初回デプロイ

**"Save and Deploy"** をクリック → ビルドログが流れる（2〜4 分）。完了後、画面上部に `https://member-site-template.<your-subdomain>.workers.dev` の URL が表示される。**この時点ではまだ Service Role key が無いので、admin 機能と一部のサーバ処理は動かない**。次の Step 7 で登録する。

### Step 7: ランタイム Secret を登録（GUI）

Step 6 で作成された Worker の管理画面に移動し、ランタイムの env が読む秘匿値（Service Role key）を **別の場所** に登録する。
...
>
> Service Role key を Build variables に書くと本番で `undefined` になる。逆に `PUBLIC_*` を Runtime に書いても、それらはクライアント JS にも inline する設計なのでビルド時に値が必要で、ランタイムだけに置いても意味がない（Step 6-2 の場所が正解）。

ランタイム Secret は **登録後すぐ反映される**ので、再デプロイは不要。ブラウザでサイトをリロードすれば admin 機能も動き始める。
...
`/admin` 画面を使うには、自分のアカウントを admin に昇格させる。
...
ブラウザで `/admin` をリロード → 管理画面に入れる。詳細手順（CLI 経路 / Option A vs B の選択）は [.claude/deployment.md「初期 admin の bootstrap」](.claude/deployment.md#初期-admin-の-bootstrap必須1-回限り) を参照。

---

## Claude Code への頼み方の例

カスタマイズ・機能追加は Claude Code に日本語で指示する。リポジトリ直下の [CLAUDE.md](CLAUDE.md) が起動時に自動で読み込まれ、Claude Code は規約（コーディング・命名・セキュリティ・DB マイグレーションの 7 ステップ等）に従って作業する。
